...
static void udc_hpm_isr(const struct device *dev)
{
	...
	if (int_status & USB_USBINTR_UE_MASK) {
		...
		if (edpt_setup_status) {
			/* Match CherryUSB: copy setup under SUTW tripwire. */
			...
			do {
				...
				memcpy(setup_data, (uint8_t *)&qhd0->setup_request, sizeof(setup_data));
			} while (!usb_dcd_get_sutw(handle->regs));
			usb_dcd_set_sutw(handle->regs, false);

			udc_hpm_handler_setup(dev, (struct usb_setup_packet *)setup_data);
		}
	}
}
